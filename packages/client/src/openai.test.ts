...
describe('OpenAIClient', () => {
  describe('generateTextWithStreaming', () => {
    ...
    it('should handle streaming connection errors', async () => {
      ...
      ).rejects.toThrow('Failed to generate text with streaming: Network connection failed');
    });
    ...
    it('should handle non-Error exceptions', async () => {
      ...
      ).rejects.toThrow('Failed to generate text with streaming: Unexpected error');
    });

    it('should call onTextChunk callback with messageId', async () => {
      const mockStream = [{ type: 'response.output_text.delta', delta: 'Test' }];
    ...
    it('should handle streaming interruption gracefully', async () => {
      const mockStream = [{ type: 'response.output_text.delta', delta: 'Start ' }];
      ...
      ).rejects.toThrow('Failed to generate text with streaming: Stream interrupted');
    });
  });
});
