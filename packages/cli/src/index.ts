...
async function agentLoop(
  ...
  while (true) {
    ...
    const shouldContinue = await handleToolCall(toolCallEvent.event, orchestrator);
...
async function generateToolCall(
  ...
  | { event: ToolCallEvent; thread: Thread; shouldExit: false }
  | { thread: Thread; shouldExit: true }
  | null
> {
  try {
