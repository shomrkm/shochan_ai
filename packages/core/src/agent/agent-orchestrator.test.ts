...
import type { ToolExecutionResult, ToolExecutor } from './tool-executor';
...
class MockToolExecutor implements ToolExecutor {
  execute = vi.fn(async (toolCall: ToolCall): Promise<ToolExecutionResult> => {
...
class ErrorMockToolExecutor implements ToolExecutor {
  execute = vi.fn(async (_toolCall: ToolCall): Promise<ToolExecutionResult> => {
