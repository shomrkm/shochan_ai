...
describe('Agent Routes', () => {
  ...
  beforeAll(async () => {
    ...
    const deps: AgentDependencies = {
      ...
      reducer: {
        ...
        reduce: (state: Thread, event: unknown) => new Thread([...state.events, event as any]),
  ...
  describe('POST /api/agent/query', () => {
    ...
    it('should return 400 when message is missing', async () => {
      const response = await request(app).post('/api/agent/query').send({}).expect(400);
  ...
  describe('POST /api/agent/approve/:conversationId', () => {
    ...
    it('should return 400 when no pending approval', async () => {
      ...
      expect(response.body.message).toBe('This conversation is not waiting for approval');
    });
