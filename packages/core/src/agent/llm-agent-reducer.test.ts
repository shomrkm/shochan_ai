...
describe('LLMAgentReducer', () => {
  describe('generateNextToolCall', () => {
    it('should generate tool call successfully', async () => {
      ...
      const mockSystemPromptBuilder = (context: string) => `System prompt: ${context}`;

      const reducer = new LLMAgentReducer(mockLLMClient, mockTools, mockSystemPromptBuilder);
  ...
  describe('generateNextToolCallWithStreaming', () => {
    it('should throw error when LLM client does not support streaming', async () => {
      ...
      await expect(reducer.generateNextToolCallWithStreaming(thread)).rejects.toThrow(
    ...
    it('should handle streaming errors gracefully', async () => {
      ...
      await expect(reducer.generateNextToolCallWithStreaming(thread)).rejects.toThrow(
        'Streaming tool call generation failed: Streaming failed'
      );
    });
  });
  ...
  describe('generateExplanationWithStreaming', () => {
    it('should throw error when LLM client does not support text streaming', async () => {
      ...
      await expect(reducer.generateExplanationWithStreaming(thread)).rejects.toThrow(
        'LLM client does not support text streaming'
      );
    });
    ...
    it('should generate explanation successfully', async () => {
      const mockLLMClient = {
        generateToolCall: vi.fn(),
        generateTextWithStreaming: vi.fn().mockResolvedValue('Task created successfully'),
      };
    ...
    it('should call onTextChunk callback for each chunk', async () => {
      ...
      expect(onTextChunk).toHaveBeenNthCalledWith(1, 'Task ', expect.any(String));
      expect(onTextChunk).toHaveBeenNthCalledWith(2, 'created ', expect.any(String));
      expect(onTextChunk).toHaveBeenNthCalledWith(3, 'successfully', expect.any(String));
    });
    ...
    it('should handle non-Error exceptions in streaming', async () => {
      const mockLLMClient = {
        generateToolCall: vi.fn(),
        generateTextWithStreaming: vi.fn().mockRejectedValue('Unexpected error'),
      };
